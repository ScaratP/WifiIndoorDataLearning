...
def create_vector_format(data, target_ssids=None):
    ...
    for record in data:
        ...
        vector_data.append({
            'id': record['id'],
            'name': record['name'],
            'x': float(record['x']) if record['x'] is not None else 0.0,
            'y': float(record['y']) if record['y'] is not None else 0.0,
            'building': building,
            'floor': floor,
            'room': room,
...
def save_to_csv(vector_data, bssid_info, output_dir):
    ...
    print(f"座標範圍: x=[{df['x'].min():.2f}, {df['x'].max():.2f}], y=[{df['y'].min():.2f}, {df['y'].max():.2f}]")
