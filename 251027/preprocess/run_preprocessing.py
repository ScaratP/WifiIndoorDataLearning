...
def main():
    ...
    # *** # *** 新增：將座標縮放資訊添加到 preprocessing_info ***
    # *** print("\n   正在將座標縮放資訊 (mean/std) 添加到 preprocessing_info...")
    preprocessing_info["coord_scaling"] = {
        "mean_x": float(dataset.lo_mean),  # 對應 Kotlin 的 coordMeanX
        "std_x": float(dataset.lo_std),    # 對應 Kotlin 的 coordStdX
        "mean_y": float(dataset.la_mean),  # 對應 Kotlin 的 coordMeanY
        "std_y": float(dataset.la_std)     # 對應 Kotlin 的 coordStdY
    }
