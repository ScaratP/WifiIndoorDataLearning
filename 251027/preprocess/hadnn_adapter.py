...
class NTTUDataset:
    ...
    '''
    def normalize_rss(self):
    #    """標準化 RSS 資料 - 改為使用 [0, 1] 縮放 (同 Android 端邏輯)"""
        print("   正在將 RSS 訊號縮放至 [0, 1] 範圍...")

        # 1. 截斷 (Clipping) 到 [-100, 0]
        #    (create_vector_format.py 已經將預設值設為 -100)
        train_clipped = np.clip(self.train_x, -100.0, 0.0)
        test_clipped = np.clip(self.test_x, -100.0, 0.0)

        # 2. 線性縮放 (Normalization) 到 [0, 1]
        self.train_x = (train_clipped + 100.0) / 100.0
        self.test_x = (test_clipped + 100.0) / 100.0

        # (可選) 同時更新原始的 rss_data (如果 run_preprocessing.py 中的分析需要)
        # self.rss_data = (np.clip(self.rss_data, -100.0, 0.0) + 100.0) / 100.0
