...
# --- ★ 修改：建立一個新的模型資料夾 ---
MODEL_DIR_BASE = "models_tuned_low" # 避免覆蓋舊結果
CONFIG_DIR = "hadnn_data_split"

# --- ★ 修改：專為「低誤差」模型特調的超參數 ★ ---
ATTENTION_UNITS = 128       # (保持不變)
NUM_HEADS = 4               # (保持不變)
DENSE_UNITS_1 = 512         # (保持不變)
DENSE_UNITS_2 = 256         # (保持不變)
DROPOUT_RATE = 0.4          # (保持不變)
L2_REG = 0.005              # (保持不變)
LEARNING_RATE = 0.00005     # <--- 調低 (從 0.0001 變 0.00005)，進行精修
EPOCHS = 1000                # <--- 延長 (從 150 變 300)，讓它慢慢收斂
BATCH_SIZE = 32
EARLY_STOPPING_PATIENCE = 25 # <--- 延長 (給它更多耐心)
LR_REDUCE_PATIENCE = 5     # <--- 延長
LR_REDUCE_FACTOR = 0.5      # (每次降速 50%)
MIN_LR = 1e-7               # <--- ★★★ 新增！(學習率最低降到 1e-7)
...
# --- ★ 修改：主訓練迴圈 ★ ---
def main():
    print("--- 開始訓練「低誤差」群組 (sea4, sea5) ---")
    
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_model_dir = os.path.join(MODEL_DIR_BASE, f"coord_run_low_error_{timestamp}")
    try:
    ...
    try:
        with open(params_filepath, 'w', encoding='utf-8') as f:
            f.write("--- Hyperparameters (Low Error Models) ---\n")
    ...
    # --- ★ 修改：只鎖定我們要的群組 ★ ---
    group_names = ["sea4", "sea5"]
    print(f"   將專注訓練低誤差群組: {group_names}")
    ...
    # --- (迴圈內的 A 到 I 步驟，全部和上面 high_error 版本一模一樣) ---
    for group_name in group_names:
        ...
        callbacks = [
            EarlyStopping(
                monitor='val_loss', 
                patience=EARLY_STOPPING_PATIENCE, # (耐心 25)
                restore_best_weights=True, 
                verbose=1
            ),
            ReduceLROnPlateau(
                monitor='val_loss',
                factor=LR_REDUCE_FACTOR, # (0.5)
                patience=LR_REDUCE_PATIENCE, # (★ 靈敏度 5)
                min_lr=MIN_LR, # (★ 最小學習率 1e-7)
                verbose=1
            )
        ]
        history = model.fit(
        ...
        print(f"   測試集損失 (MSE): {eval_loss:.4f}, MAE (Scaled): {eval_mae:.4f}")
        ...
        if not scaler_params:
            ...
            mean_x, std_x = scaler_params['mean_x'], scaler_params['std_x']
            mean_y, std_y = scaler_params['mean_y'], scaler_params['std_y']
    ...
    try:
        with open(params_filepath, 'a', encoding='utf-8') as f:
            f.write("\n\n--- Training Results (Low Error Groups) ---\n")
    ...
    print(f"\n--- ✅ 低誤差模型訓練流程結束 (結果儲存於 {run_model_dir}) ---")
